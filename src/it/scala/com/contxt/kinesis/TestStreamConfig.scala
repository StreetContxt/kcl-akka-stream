...
case class TestStreamConfig(
  ...
  def kclConfig(workerId: String): ConsumerConfig = {
    val kinesisClient = KinesisClientUtil.createKinesisAsyncClient(KinesisAsyncClient.builder().region(Region.of(regionName)))
    val initialPositionInStream = InitialPositionInStreamExtended.newInitialPosition(InitialPositionInStream.TRIM_HORIZON)
    ConsumerConfig(
      streamName,
      applicationName,
      workerId,
      ...
      coordinatorConfig = Some(new CoordinatorConfig(applicationName).
